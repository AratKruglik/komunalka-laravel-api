...
- **Pest PHP v4** — BDD-style syntax (`describe()` + `it()` + `expect()`)
...
- **JWT auth in tests** — `actingAs($user, 'api')`
...
│   ├── ApiCompatibility/     # Response format, pagination, token structure
│   ├── EndpointCoverage/     # Edge cases, security, photo serving
│   └── Performance/          # N+1 prevention, queue processing
├── Unit/                     # Unit tests (Actions, Observers, Support)
...
See `@.gemini/rules/docker-commands.md` for the full command list.

## Test Configuration

- **Database**: RefreshDatabase trait for clean state (separate PostgreSQL instance for tests)
- **Environment**: phpunit.xml with testing-specific settings
- **Coverage**: reports in `coverage/` directory
- **Memory Limit**: 512M
- **Parallel mode**: 12 processes by default
...
- Use `route()` helper with named routes — never hardcode URLs
- Use model factories with custom states; follow existing `$this->faker` vs `fake()` conventions
- Do NOT delete tests without approval
- Most tests should be feature tests; use unit only for pure logic with no IO

> For writing test code (describe/it/expect, JWT actingAs, mutation, browser tests): activate skill `pest-testing`.
