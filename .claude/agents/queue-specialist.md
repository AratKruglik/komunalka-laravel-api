...
class SendMentorProgramNotification implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        $program = MentorProgram::query()->findOrFail($this->programId);
...
use App\Jobs\SendMentorProgramNotification;
use Lorisleiva\Actions\Concerns\AsObject;

class NotifyMentorProgramParticipants
{
    use AsObject;

    public function handle(MentorProgram $program): void
    {
        $program->participants()->each(function ($participant) use ($program): void {
            SendMentorProgramNotification::dispatch(
                $participant->getKey(),
                $program->getKey(),
            );
        });
...
| Retry strategies | API endpoints | Supervisor config |
