...
| **403** | Policy returns false (CalendarEventPolicy, MentorProgramPolicy, UserSchedulePolicy) |
| **404** | Wrong route name, missing model, route model binding failure |
| **405** | Wrong HTTP method, route definition mismatch |
| **419** | CSRF token expired (unlikely in JWT API) |
...
### API Response Issues
- **Wrong JSON Structure**: Check JSON Resource formatting
- **JWT Token Expired**: Check token TTL and refresh flow
...
| Reproduction strategy | API endpoints | TDD workflows |
...
- **Policies for 403** — check CalendarEventPolicy, MentorProgramPolicy, UserSchedulePolicy
