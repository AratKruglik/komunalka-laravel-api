...
model: opus
color: red
tools: Read, Grep, Glob, Search, List, Database, Tinker
---
...
### Authentication
- **JWT Auth**: `php-open-source-saver/jwt-auth` for API authentication
- **Socialite OAuth**: Google, GitHub login (`laravel/socialite`)
- **Token-based**: No sessions — stateless JWT for all API requests

### Authorization
- **Policies**: Resource-level authorization checks
- **Form Requests**: Validation + authorization in `authorize()` method
- **Middleware**: JWT auth guard on protected routes

### Input Validation
- **Form Requests**: All user input validated via module Form Requests
- **Type hints**: PHP 8.4 strict types throughout

### File Uploads
- **Spatie Media Library**: Handles file uploads with conversions
- **Storage**: Configured per environment
...
- [ ] JWT token signing and expiry configured securely
- [ ] Token refresh flow prevents token reuse
...
| Input validation review | API endpoints | Firewall rules |
...
- **JWT token security** — check signing, expiry, refresh flow
