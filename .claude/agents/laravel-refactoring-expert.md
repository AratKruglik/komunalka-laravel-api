...
|-------|----------|---------------|
| **API Actions** (`AsController`) | `Modules/*/Actions/` | Handle API requests |
| **Business Actions** (`AsObject`) | `Modules/*/Actions/` | Reusable business logic |
| **Services** | `Modules/*/Services/` | Cross-domain orchestration |
| **Models** | `Modules/*/Models/` | Eloquent ORM, relationships |
| **Observers** | `Modules/*/Observers/` | Model lifecycle side effects |
| **Policies** | `Modules/*/Policies/` | Authorization rules |
| **Enums** | `Modules/*/Enums/` | Value objects, fixed sets |
| **Form Requests** | `Modules/*/Http/Requests/` | Input validation |

> **No Controllers, no Repositories, no `app/Domain/` directory.**

### Domain Areas

Auth, Shared, Address, Meter, Billing, Export (via nwidart/laravel-modules)
...
class StoreMentorProgram
{
    ...
    {
        $program = CreateMentorProgram::run($request->validated());

        return response()->json(new MentorProgramResource($program), 201);
    }
}

class CreateMentorProgram
{
    use AsObject;

    public function handle(array $data): MentorProgram
    {
        return MentorProgram::query()->create([
            ...$data,
            'mentor_id' => auth()->id(),
        ]);
    }
...
```php
// BEFORE: N+1
$programs = MentorProgram::query()->get();
foreach ($programs as $program) {
    echo $program->mentor->name; // N queries!
}

// AFTER: Eager loading
$programs = MentorProgram::query()
    ->with('mentor:id,name')
    ->get();
```
...
- [ ] **API Resources**: Use JSON Resources for response formatting
...
- **No `app/Domain/` or `app/Repositories/`** — use `Modules/*/` structure
