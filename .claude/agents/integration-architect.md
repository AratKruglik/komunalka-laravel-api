...
| **Scramble** | `dedoc/scramble` | OpenAPI documentation |
| **JWT Auth** | `php-open-source-saver/jwt-auth` | JWT authentication |
| **League CSV** | `league/csv` | CSV export |
| **Spatie PDF** | `spatie/laravel-pdf` | PDF generation |
...
namespace Modules\Billing\Actions\Webhooks;
...
All routes live in `routes/api.php` or module route files:
...
| API client wrappers | API endpoints | Server configuration |
