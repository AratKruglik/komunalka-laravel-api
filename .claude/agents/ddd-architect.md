...
├─────────────────────────────────────┤
│  Page Actions (AsController)        │  ← HTTP → JSON
│  Store/Update Actions (AsController)│  ← Form handling
├─────────────────────────────────────┤
│  Business Actions (AsObject)        │  ← Reusable logic
│  Services (CalendarService, etc.)   │  ← Cross-domain orchestration
├─────────────────────────────────────┤
...
> Business logic lives in **Actions** (`Modules/*/Actions/`) and **Services** (`Modules/*/Services/`).

### Domain Areas

| Domain | Models | Actions Location | Key Patterns |
|--------|--------|-----------------|--------------|
| **Auth** | User | `Modules/Auth/Actions/` | Socialite OAuth (Google, GitHub), JWT |
| **Shared** | UtilityType, Currency, ServiceCategory | `Modules/Shared/Actions/` | Reference data, Enums |
| **Address** | Address, Region | `Modules/Address/Actions/` | Address CRUD, Regions |
| **Meter** | Meter, MeterReading | `Modules/Meter/Actions/` | Meters, Readings, Photos |
| **Billing** | ServiceProvider, Tariff | `Modules/Billing/Actions/` | Service Providers, Tariffs |
| **Export** | — | `Modules/Export/Actions/` | CSV (League CSV), PDF (Spatie PDF) |

### Patterns In Use

| Pattern | Location | Purpose |
|---------|----------|---------|
| **Actions (AsController)** | `Modules/*/Actions/` | Page rendering, form handling |
| **Actions (AsObject)** | `Modules/{Module}/Actions/` | Reusable business logic |
| **Services** | `Modules/*/Services/` | Cross-domain orchestration |
| **Observers** | `Modules/*/Observers/` (or `app/Observers/`) | Model lifecycle side effects |
| **Policies** | `Modules/*/Policies/` | Authorization (CalendarEvent, MentorProgram, UserSchedule) |
...
| Model lifecycle hooks | **Observer** | `CalendarEventObserver` |
...
class ListMeters
{
    ...
    public function handle(): AnonymousResourceCollection
    {
        $meters = Meter::query()
            ...
            ->with(['address:id,street,building'])
...
use Modules\Shared\Enums\TagEnum;
...
| Architecture decisions | JSON Resources | Migration content |
| Logic placement | Form handling | Index strategy |
| Pattern selection | API endpoints | Query optimization |
| Event design | API endpoints | Relationship modeling |
...
- **No `app/Domain/` directory** — use `Modules/{Module}/Actions/`
